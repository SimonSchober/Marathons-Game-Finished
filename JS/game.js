class Game {
    ...
    async start() {
        ...
        Water = new Group()
        DogCrew = new Group()
        this.addSprites(Water, 20, WaterIMG, 0.3)
        this.addSpritesAn(DogCrew, 5, dogIMG, 0.7)
        // this.addSprites(Dog, 5, DogIMG, 0.5)
    }
    ...
    addSpritesAn(spriteGroup, NoOfSprites, SpriteAnimation, Scale){
        for (var i = 0; i < NoOfSprites; i++) {
            var x, y
            x = random(width - 400, width * 5 - 200)
            y = random(height / 2 - 300, height / 2 + 300)
            var Sprite = createSprite(x, y)
            Sprite.addAnimation("running" ,SpriteAnimation)
            Sprite.scale = Scale
            Sprite.velocityX = -5
            spriteGroup.add(Sprite)
        }
